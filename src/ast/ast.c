...
E Ast_Exp_new_bop (Ast_Exp_Kind_t kind
                   , E left
                   , E right
                   , Type_t ty
                   , Region_t r)
...
File_t Ast_Exp_print (File_t file, E e)
{
  ...
  case AST_EXP_NEW_CLASS:{
    ...
    fprintf (file, "%s", "new ");
    fprintf (file, "%s"
             , AstId_toString(e->u.newClass.name));
    fprintf (file, "%s", "(");
    while (p){
      E f = (E)p->data;
...
File_t Ast_Block_print (File_t file, B b)
{
  ...
  List_foldl (b->decs
              , file
              , (Poly_tyFold)Ast_Dec_print);
  List_foldl (b->stms
              , file
              , (Poly_tyFold)Ast_Stm_print);
  unindent ();
  spaces (file);
  fprintf (file, "%s", "}");
  return file;
...
void Ast_Prog_print (File_t file, P x)
{
  ...
  List_foldl (x->classes
              , file
              , (Poly_tyFold)Ast_Class_print);
  fprintf (file, "%s", "\n");
  List_foldl (x->funcs
              , file
              , (Poly_tyFold)Ast_Fun_print);
  return;
}
