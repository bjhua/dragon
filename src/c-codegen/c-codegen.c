...
static void outputFrame (Machine_FrameInfo_t p)
{
  ...
  fprintf (file, "_%d[] = {%d ", index
           , List_size (p->frameOffsets));
  while (tmp){
...
static void outputStm (Machine_Stm_t s)
{
  ...
  case MACHINE_STM_TRY_END:{
    ...
    fprintf (file, "extern int %s%s;\n"
             ...
             , Label_toString(s->u.tryEnd));
    fprintf (file, "Dragon_Exn_end ((int)&%s%s);"
             , Label_toString(s->u.tryEnd)
             , Label_toString(s->u.tryEnd));
...
C_codegenTraced (Machine_Prog_t p)
{
  List_t frame;
  List_t layouts;
