...
{
  struct Lval_Result_t result = {Type_new_int ()
                                 , l};
  ...
  case AST_LVAL_VAR:{
    ...
    result.type = bd->type;
    result.lval = Ast_Lval_new_var (bd->fresh
                                    , result.type
                                    , l->region);
    return result;
  }
  case AST_LVAL_DOT:{
    ...
    result.lval = Ast_Lval_new_dot (left.lval
                                    , bdVar->fresh
                                    , result.type
                                    , l->region);      
    return result;
  ...
  case AST_LVAL_ARRAY:{
    ...
    if (!r.type->isArray){
      printf ("expected: array type\n"
              "but got : %s", Type_toString (r.type));
      error ("", l->region);
      result.type = Type_new_int ();
    }
    else result.type = Type_clearArray (r.type);
...
static struct Exp_Result_t Elab_exp (Ast_Exp_t e)
{
  ...
  case AST_EXP_ASSIGN:{
    ...
    result.exp = Ast_Exp_new_assign (r1.exp
                                     , r2.exp
                                     , r1.type
                                     , e->region);
    result.type = r1.type;
  ...
  case AST_EXP_NEW_CLASS:{
    ...
    if (List_isEmpty (e->u.newClass.args)){      
      ...
      result.exp = 
        Ast_Exp_new_newClass (freshClassName
                              , e->u.newClass.args
                              , result.type);
      return result;
    }
      ...
      ptr_field = List_getFirst 
        (Cenv_lookupFields(oldClassName));
      ...
      result.exp = Ast_Exp_new_newClass (freshClassName
                                         , newFields
                                         , result.type);
      return result;
    }
  ...
  case AST_EXP_NEW_ARRAY:{
    ...
    result.type = ty;
    printf ("%s\n", Type_toString (ty));
    result.exp = Ast_Exp_new_newArray (newTy
                                       , Type_new_array (ty)
                                       , r.exp);    
    return result;
  }
...
{
  AstId_t newClassName, newFieldName = 0;
  Ast_Type_t newType;
  Type_t semType;
  ...
  newFieldName = Senv_insert (oldClassName
                              , dec->var
                              , semType);
  // check the "init" field to be 0
  Assert_ASSERT(dec->init == 0);
...
{
  AstId_t name, newClassName;
  List_t newFields;
  
  Assert_ASSERT(c);
  oldClassName = c->name;
  newClassName = Cenv_lookupMustExist (c->name);
  newFields = List_map (c->fields
                        , (Poly_tyId)Elab_fieldEach);
