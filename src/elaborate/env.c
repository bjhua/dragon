...
static void error_dupClassName(Id_t old, AstId_t current) {
    UNUSED(old);
    error(String_concat("duplicate class name: ",
...
AstId_t Cenv_lookupMustExist(AstId_t id) {
    CEnv_Binding_t b = (CEnv_Binding_t) Hash_lookup(cenv, id);
    if (!b) {
        error(String_concat("undefined class name: ",
                            AstId_toString(id),
                            0),
              AstId_getRegion(id));
        return id;
...
static void error_dupVar(AstId_t old, AstId_t cur) {
    UNUSED(old);
    error(String_concat("duplicate variable: ",
                        AstId_toString(cur),
                        0),
          AstId_dest(cur));
    return;
...
void Venv_init(void) {
    ...
    /* build all libraries */
    {
        ...
        Hash_insert(h,
                    idIo_printi,
                    Env_Binding_new(Type_new_fun(Type_new_product(Type_new_int(),
                                                                  0),
                                                 Type_new_int()),
                                    idIo_printi));
        Stack_push(venv, h);
    }
    {
        ...
        Hash_insert(h,
                    idIo_print,
                    Env_Binding_new(Type_new_fun(Type_new_product(Type_new_string(),
                                                                  0),
                                                 Type_new_int()),
                                    idIo_print));
        Stack_push(venv, h);
...
Env_Binding_t Venv_lookupMustExist(AstId_t id) {
    ...
    b = Venv_lookup(id);
    if (!b) {
        error(String_concat("unbound variable: ",
                            AstId_toString(id),
                            0),
              AstId_dest(id));
...
static long Skey_hashCode(Tuple_t k) {
    assert(k);
    return AstId_hashCode(Tuple_first(k)) + AstId_hashCode(Tuple_second(k));
}

static int Skey_equals(Tuple_t k1, Tuple_t k2) {
    assert(k1);
    assert(k2);
    return AstId_equals(Tuple_first(k1), Tuple_first(k2)) && AstId_equals(Tuple_second(k1), Tuple_second(k2));
}
...
static void error_dupField(Tuple_t k, AstId_t id) {
    ...
    error(String_concat("duplicate class field: ",
                        AstId_toString(fid),
                        " in ",
                        AstId_toString(sid),
                        0),
...
Env_Binding_t Senv_lookupMustExist(AstId_t sid, AstId_t fid) {
    ...
    if (!bd) {
        error(String_concat("unbound class field: ",
                            AstId_toString(fid),
                            0),
              AstId_dest(fid));
        return Env_Binding_new(Type_new_int(),
