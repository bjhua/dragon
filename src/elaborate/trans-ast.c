...
static struct Exp_Result_t Elab_exp (Ast_Exp_t e)
{
  ...
  case AST_EXP_ASSIGN:{
    ...
    newAssign = Hil_Stm_new_assign (newLeft.exp->u.lval
                                    , newRight.exp);
    result.list = 
      AppList_new_list (List_list 
                        (newRight.list
                         , newLeft.list
                         , AppList_fromItem (newAssign)
                         , 0));
    return result;
  }
  case AST_EXP_ADD:
  ...
  case AST_EXP_NE:{
    ...
    result.exp = 
      Hil_Exp_new_bop (convertOperator (e->kind)
                       , newLeft.exp
                       , newRight.exp
                       , type2atype (e->ty));
    result.list = AppList_concat (newLeft.list
                                  , newRight.list);
    return result;
  }
  case AST_EXP_NOT:
  ...
  case AST_EXP_INTLIT:{    
    result.exp = Hil_Exp_new_intlit (e->u.intlit
                                     , Atype_new_int ());
    result.list = AppList_new_empty ();
    return result;
  }
  ...
  case AST_EXP_CALL:{
    ...
    result.exp = Hil_Exp_new_call (fname
                                   , tempExps
                                   , ty
                                   , leave
                                   , normal);
...
static List_t Elab_stm (Ast_Stm_t s)
{
  ...
  case AST_STM_FOR:{
    ...
    List_insertLast (newTailx
                     , Hil_Stm_new_exp (newTail.exp));
    List_append (newTailx, newCondx);
    
    List_append (result, AppList_toList (newHeader.list));
    List_insertLast (result
                     , Hil_Stm_new_exp (newHeader.exp));
    List_append (result, AppList_toList (newCond.list));
    List_insertLast 
      (result
       , Hil_Stm_new_if(newCond.exp
                        , List_list (Hil_Stm_new_do
                                     (newCond.exp
                                      , newBody
                                      , Tuple_first (tuple)
                                      , Tuple_second (tuple)
  ...
  case AST_STM_BREAK:{
    ...
    return List_list (Hil_Stm_new_jump (exitLabel)
                      , 0);
  }
  case AST_STM_CONTINUE:{
    Tuple_t tuple = peekLabel ();
    Label_t entryLabel = Tuple_first (tuple);
    return List_list (Hil_Stm_new_jump (entryLabel)
                      , 0);
  }
  case AST_STM_RETURN:{
    struct Exp_Result_t er;
  ...
  case AST_STM_THROW:{
    ...
    if (Stack_isEmpty (exnLabels)){
      return List_list (Hil_Stm_new_throw ()
                        , 0);
    }
    label = peekExnLabel ();
    return List_list (Hil_Stm_new_localThrow (label)
                      , 0);
  }
  case AST_STM_TRYCATCH:{
    ...
    return List_list (Hil_Stm_new_tryCatch (newTry
                                            , newCatch
                                            , label
                                            , Label_new())
                      , 0);
...
{
  Assert_ASSERT(dec);
  return Dec_new (asttype2atype (dec->type)
                  , AstId_toId (dec->var));
}
...
{
  return Class_new (AstId_toId (c->name)
                    , List_map(c->fields
                               , (Poly_tyId)Elab_dec));
}
...
{
  File_saveToFile ("trans-ast.result"
