...
File_t Hil_Exp_print (File_t file, E e)
{
  ...
  case HIL_EXP_UOP: 
    fprintf (file, "%s"
             , Operator_toString (e->u.unary.op));
  ...
  case HIL_EXP_NEW_CLASS:
    ...
    fprintf (file, "%s", "(");
    List_foldl (e->u.newClass.args
                , file
                , (Poly_tyFold)Hil_Exp_print);
    fprintf (file, "%s", ");");
    break;
  case HIL_EXP_NEW_ARRAY:
    fprintf (file, "%s", "new ");
    fprintf (file, "%s"
             , Atype_toString (e->u.newArray.type));
    fprintf (file, "%s", "[");
  ...
  case HIL_EXP_CALL:
    ...
    fprintf (file, "%s", "(");
    List_foldl (e->u.call.args
                , file
                , (Poly_tyFold)Hil_Exp_print);
    fprintf (file, "%s", ")");
    fprintf (file, "LEAVE = %s, NORMAL = %s"
             , (e->u.call.leave)
             ? Label_toString (e->u.call.leave): "<LEAVE>"
             , (e->u.call.normal)?
             Label_toString (e->u.call.normal): "<NORMAL>");
    break;
  case HIL_EXP_LVAL:
...
File_t Hil_Stm_print (File_t file, S s)
{
  ...
  case HIL_STM_IF:
    ...
    fprintf (file, ")\n");
    indent ();
    List_foldl (s->u.iff.then
                , file
                , (Poly_tyFold)Hil_Stm_print);
    unindent ();
    spaces (file);
    fprintf (file, "else\n");
    indent ();
    List_foldl (s->u.iff.elsee
                , file
                , (Poly_tyFold)Hil_Stm_print);
    unindent ();
    break;
  case HIL_STM_DO:
    ...
    fprintf (file, " Lexit=");
    fprintf (file, "%s"
             , Label_toString (s->u.doo.exitLabel));
  ...
  case HIL_STM_TRYCATCH:{
    ...
    fprintf (file, "%s", "\n");
    List_foldl (s->u.tryCatch.catchh
                , file
                , (Poly_tyFold)Hil_Stm_print);
    spaces (file);
    fprintf (file, "%s\n", "}");
