...
static void printLabel (Label_t l)
{
  ...
  printf (": ");
  if (labelInfoProp){
    String_t info = Property_get (labelInfoProp
                                  , l);
...
static void Cache_log ()
{
  ...
  while (p){
    ...
    case STM:
      Log_fun (data->u.stm
               , (Poly_tyLog)Ssa_Stm_print);
      break;
    case TRANS:
      Log_fun (data->u.trans
               , (Poly_tyLog)Ssa_Transfer_print);
      break;
    default:
      Error_impossible ();
      return;
...
static Id_t Trans_lvalRight (Hil_Lval_t l)
{
  ...
  case HIL_LVAL_DOT:{
    ...
    Assert_ASSERT(l->ty);
    fresh = genDec (l->ty);
    emitStm (Ssa_Stm_new_load (fresh
                               , mem));
    return fresh;
  }
  case HIL_LVAL_ARRAY:{
...
static Ssa_Operand_t Trans_exp (Hil_Exp_t e)
{
  ...
  case HIL_EXP_CALL:{
    ...
    emitTrans (Ssa_Transfer_new_call (result
                                      , e->u.call.f
                                      , newArgs
                                      , e->u.call.leave
                                      , normal));
...
static void Trans_stm (Hil_Stm_t s)
{
  ...
  case HIL_STM_DO:{
    ...
    emitTrans (Ssa_Transfer_new_if (result
                                    , start
                                    , s->u.doo.exitLabel));
    if (Control_labelInfo)
      Property_set (labelInfoProp
...
static struct Block_Result_t cookBlocks () 
{
  ...
  while (p){
    ...
    case TRANS:{
      ...
      block = Ssa_Block_new (firstLabel
                             , stms
                             , newTransfer);
...
static Ssa_Prog_t Hil_transTraced (Hil_Prog_t p)
{
  ...
  fieldProp = Property_newInitFun 
    ((Poly_tyPlist)Id_plist
     , (Poly_tyPropInit)fieldPropInitFun);
  ...
  return Ssa_Prog_new (p->classes
                       , newFuncs);
}

static void printArg (Hil_Prog_t p)
{
  File_saveToFile ("trans_hil.arg"
...
{
  File_saveToFile ("trans_hil.result"
