...
static Coordinate_t getPos ()
{
  return Coordinate_new (pos.fname
                         , pos.line
                         , pos.column-1);
}
...
static void error (char *msg)
{
  ErrorMsg_lexError (msg
                     , pos.fname
                     , pos.line
                     , pos.column);
}

static void error2 (String_t msg
                    , String_t fname
                    , int line
                    , int column)
{
...
static Token_t cookNum (int c, Coordinate_t leftPos)
{
  ...
  return Token_new (TOKEN_INTLIT
                    , CharBuffer_toStringBeforeClear 
                    (numBuffer)
                    , leftPos
                    , getPos ());
}
...
static int escape (int c)
{
  ...
  case '\"':
    return '\"';
  default:
    ErrorMsg_lexError ("bad escape sequence"
                       , pos.fname
                       , pos.line 
                       , pos.column);
    return 0;
...
static Token_t cookId (int c, Coordinate_t leftPos)
{
  ...
  kind = isKeyWord (str);
  if (kind > 0)
    return Token_new (kind
                      , 0
                      , leftPos
                      , getPos());
  return Token_new (TOKEN_ID
                    , str
                    , leftPos
                    , getPos ());
}
...
Token_t Lex_getTokenTraced () 
{
  ...
  case '<':{
    ...
    switch (secondChar) {
    case '=':
      return Token_new (TOKEN_LE
                        , 0
                        , leftPos
                        , getPos ());
    default:
      Lex_putChar ();
      return Token_new (TOKEN_LT
                        , 0
                        , leftPos
                        , getPos ());
    }
  ...
  case '>':{
    ...
    switch (secondChar) {
    case '=':
      return Token_new (TOKEN_GE
                        , 0
                        , leftPos
                        , getPos ());
    default:
      Lex_putChar ();
      return Token_new (TOKEN_GT
                        , 0
                        , leftPos
                        , getPos ());
    }
  ...
  case '+':
    return Token_new (TOKEN_ADD
                      , 0
                      , leftPos
                      , getPos ());
  case ';': 
    return Token_new (TOKEN_SEMI
                      , 0
                      , leftPos
                      , getPos ());
  case ',': 
    return Token_new (TOKEN_COMMER
                      , 0
                      , leftPos
                      , getPos ());
  case '*': 
    return Token_new (TOKEN_TIMES
                      , 0
                      , leftPos
                      , getPos ());
  case '%': 
    return Token_new (TOKEN_PERCENT
                      ...
                      , getPos ());
  case '[':
    return Token_new (TOKEN_LBRACK
                      , 0
                      , leftPos
                      , getPos ());
  case ']': 
    return Token_new (TOKEN_RBRACK
                      , 0
                      , leftPos
                      , getPos ());
  case '{': 
