...
void Graph_toJpgWithName (T g
                          , Poly_tyPrint printer
                          , String_t fname)
{
  Dot_t d = Dot_new (printer);
  String_t name = String_concat (g->name
                                 , fname
                                 , 0);  
...
{
  dprintf ("%s, "
          , Label_toString (((Ssa_Block_t)(v->data))->label));
}
...
static void printDoms (T g, Property_t domProp)
{
  ...
  while (vs){
    V from = (V)vs->data;    
    Set_t set = Property_get (domProp
                              , from);
    // hard-coded here
    dprintf ("%s ==>"
            , Label_toString (((Ssa_Block_t)(from->data))->label));
    if (!set)
      Error_impossible ();
...
static void initDom (T g, V start, Property_t dom)
{
  ...
  while (vs){
    ...
    Property_set (dom
                  , v
                  , Set_fromList 
                  ((Poly_tyEquals)Vertex_equals
                   , g->vs));
    vs = vs->next;
  }
  Property_set (dom
                , start
                , Set_singleton ((Poly_tyEquals)Vertex_equals
                                 , start));
  return;
}
...
static int idomDeletable (V v)
{
  return (int)Property_get (globalDeletable, v);
}
...
static Tree_t Graph_domTreeReal (T g
                                 , Poly_t start
                                 , Property_t *domProp
                                 , Property_t *idomProp)
{
  ...
  idom = Property_newInitFun ((Poly_tyPlist)Vertex_plist
                              , (Poly_tyPropInit)idomPropInitFun);
  ...
  while (changed){
    ...
    while (vs){
      ...
      Set_t domSetCurrent = Property_get (dom
                                          , current);
      ...
      while (predList){
        ...
        result = Set_intersection (result
                                   , domforP);        
  ...
  // contruct tree
  {
    ...
    tree = Tree_newWithName ((Poly_tyEquals)Vertex_equals
                             , String_concat (g->name
                                              , "domTree"
                                              , 0));
...
// Return: Set<V>
static Set_t Graph_dfDoit (T g
                           , V n
                           , Tree_t domTree
                           , Property_t dom
                           , Property_t idom
                           , void (*markDf)(Poly_t, Set_t))
{
  // Set<V>
  ...
  // mark df set on data
  printf ("%s ==> ["
          , Label_toString (((Ssa_Block_t)(n->data))->label));
  {
...
Tree_t Graph_df (T g, Poly_t start
               , void (*markDf)(Poly_t, Set_t))
{
  // V -> Set_t<V>
  Property_t domProp 
  ...
  Property_t idomProp
    = Property_new ((Poly_tyPlist)Vertex_plist);
  // Tree<V>
  Tree_t domTree = Graph_domTreeReal (g
                                      , start
                                      , &domProp
                                      , &idomProp);
  ...
  Graph_dfDoit (g, startv, domTree, domProp
                ,idomProp, markDf);
  ...
  return Tree_map (domTree, g->equals
                   , (Poly_tyId)localMap);
}
