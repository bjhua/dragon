...
int Lib_testTraced (int junk)
{
  // test set
  {
    ...
    Set_foreach (Set_intersection (set, set2)
                 , (Poly_tyVoid)String_print);

    printf (" \\/ = ");
    Set_foreach (Set_union(set, set2)
                 , (Poly_tyVoid)String_print);
  ...
  // of Muchnick.
  {
    ...
    */
    Graph_t g 
      = Graph_newWithName((Poly_tyEquals)String_equals
                          , "test");
    Tree_t tree;
