...
static Machine_Mem_t genMem (Machine_Mem_t m)
{
  ...
  case MACHINE_MEM_CLASS:{
    int index = (int)Property_get (fieldProp
                                   , m->u.class.field);
    return Machine_Mem_new_class (m->u.class.name
                                  , m->u.class.field
                                  , index);
  }
  default:
...
static Machine_Stm_t genStm (Machine_Stm_t s)
{
  ...
  case MACHINE_STM_STORE:{
    return Machine_Stm_new_store (genMem (s->u.store.m)
                                  , s->u.store.src);
  }
  case MACHINE_STM_LOAD:
    return Machine_Stm_new_load (s->u.load.dest
                                 , genMem (s->u.load.m));
  case MACHINE_STM_NEW_CLASS:{
    int index = (int)Property_get (indexProp
                                   , s->u.newClass.cname);
    int size = (int)Property_get (sizeProp
                                  , s->u.newClass.cname);
    
    return Machine_Stm_Runtime_class (s->u.newClass.dest
                                      , index
                                      , size
                                      , Runtime_class);
  }
...
Machine_genLayoutTraced (Machine_Prog_t p)
{
  ...
  return Machine_Prog_new (p->strings
                           , p->frameInfo
                           , layouts
                           , p->classes
                           , newFuncs);
