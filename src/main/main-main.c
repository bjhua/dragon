...
static int assembleOne (String_t file)
{
  ...
  objFile = String_concat (Control_asmDirectory
                           , "fileo-"
                           , Int_toString(i++)
                           , ".o"
                           , 0);
  List_insertLast (objFiles, objFile);
  cmd1 = String_concat ("gcc -c -g "
                        , "-I \""
...
static String_t link (List_t files)
{
  ...
  Assert_ASSERT(files);
  exeFile = (Control_o)?
    String_concat (Control_o, ".exe", 0): 
    ("a.exe");
  libFile = String_concat (Control_libDirectory
                           , "main.a"
                           , 0);
  cmd = String_concat ("gcc -g -o \""
                       , exeFile
                       , "\""
                       , 0);
  if (Control_Verb_order 
...
static int Main_main0 (List_t files)
{
  ...
  compile = Pass_new ("compile"
                      , VERBOSE_PASS
                      , files
                      , (Poly_tyId)Compile_compile);
  asmFiles = Pass_doit (&compile);

  assemblePass = Pass_new ("assemble"
                           , VERBOSE_PASS
                           , asmFiles
                           , (Poly_tyId)assemble);
...
int Main_main (int argc, char **argv)
{
  ...
  mainp = Pass_new ("dragon"
                    , VERBOSE_PASS
                    , files
                    , (Poly_tyId)Main_main0);
  Pass_doit (&mainp);
