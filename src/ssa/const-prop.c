...
static void analyzeStm (Ssa_Stm_t s)
{
  ...
  case SSA_STM_BOP:{
    ...
    if (left->kind == SSA_OP_INT
      ...
      r = Operator_binary (left->u.intlit
                           , s->u.bop.op
                           , right->u.intlit);
      
      Property_set (constProp
                    , dest
                    , Ssa_Operand_new_int (r));
    }
    // an integer and an id
    else if (left->kind == SSA_OP_INT
             && right->kind == SSA_OP_ID){
      Ssa_Operand_t rv = Property_get (constProp
                                       , right->u.id);
      
      if (rv && (rv->kind == SSA_OP_INT)){
        int r = Operator_binary (left->u.intlit
                                 , s->u.bop.op
                                 , rv->u.intlit);
        mark ();
    ...
    // and id and an integer
             ...
             && right->kind == SSA_OP_INT){
      Ssa_Operand_t lv = Property_get (constProp
                                       , left->u.id);
      
      if (lv && (lv->kind == SSA_OP_INT)){
        int r = Operator_binary (lv->u.intlit
                                 , s->u.bop.op
                                 , right->u.intlit);
        mark ();
  ...
  case SSA_STM_UOP:{
    ...
    // id and marked
      ...
      Ssa_Operand_t v = Property_get (constProp
                                      , src->u.id);
      if (v && (v->kind==SSA_OP_INT)){
        int r = Operator_unary (s->u.uop.op
                                , v->u.intlit);
...
// to perform.
static List_t rewriteStmEach (List_t result
                              , Ssa_Stm_t stm)
{
  Ssa_Stm_t new = Ssa_Stm_renameUse2Op (stm
                                        , rewriteOne);
  if (new)
...
static Ssa_Block_t rewriteBlock (Ssa_Block_t b)
{
  ...
  List_foldl (b->stms
              , newStms
              , (Poly_tyFold)rewriteStmEach);
  newTrans = Ssa_Transfer_renameUse2Op (b->transfer
                                        , rewriteOne);
