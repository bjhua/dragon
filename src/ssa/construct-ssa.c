...
static Ssa_Fun_t renameVar (Ssa_Fun_t f
                            , Graph_t g
                            , Tree_t tree);
...
static Ssa_Block_t markDefSitesOneTraced (Ssa_Block_t b)
{
  ...
  while (defIdsList){
    ...
    blockSet = Property_get (defSitesProp
                             , id);
...
static void markDefSitesOne (Ssa_Block_t b)
{
  ...
  Trace_TRACE("markDefSites"
              , markDefSitesOneTraced
              , (b)
              , dumb
              , r
              , dumb);
...
{
  Id_t id = dec->id;
  Set_t blocks = Property_get (defSitesProp
                               , id);
  List_t blockList;
  

  Log_strs (Id_toString (id)
           , " ==> ["
           , 0);
  ...
  while (blockList){
    ...
    Log_strs (Label_toString (b->label)
              , ", "
              , 0);
...
{
  Set_t vs = Property_get (origVarsProp
                           , b);
...
{
  Set_t vs = Property_get (phiVarsProp
                           , b);
...
static void printDf (Ssa_Block_t b)
{
  ...
  while (dfList){
    ...
    Log_strs (Label_toString (x->label)
              , ", "
              , 0);
    dfList = dfList->next;
  }
...
static List_t realInsertPhi (List_t bs, Graph_t g)
{
  ...
  while (blocks){
    ...
    while (phiVarsList){
      ...
      Ssa_Stm_t phi 
        = Ssa_Stm_new_phi_predsBlock (var
                                      , preds);   
...
static Ssa_Fun_t insertPhiAndRename (Ssa_Fun_t f)
{
  ...
  defSitesProp
    = Property_newInitFun 
    ((Poly_tyPlist)Id_plist
     , (Poly_tyPropInit)defSitesPropInitFun);
  ...
  phiVarsProp 
    ...
    ((Poly_tyPlist)Ssa_Block_plist
     , (Poly_tyPropInit)phiVarsPropInitFun);
  ...
  tree = Graph_df 
    (g
     , (Poly_t)Ssa_Fun_searchLabel (f
                                    , f->entry)
     , (void (*)(Poly_t, Set_t))markDf);
  ...
  Log_strs ("num of blocks in this function ="
            , Int_toString (List_size (blocks))
            , "\n"
            , 0);
  
  // check point
  Log_dot ((Poly_tyDot)Ssa_Fun_toDot, tempf, "insertphi");
  File_saveToFile ("insertPhi"
                   , (Poly_tyPrint)Ssa_Fun_print
                   , tempf);
...
static void callBackSubstPhiProp (Ssa_Block_t current
                                  , Ssa_Block_t pred
                                  , Id_t oldid)
{
  Id_t new = use (oldid);
...
static void renameVarDoit (Ssa_Block_t n)
{
  ...
  newBlock = Ssa_Block_renameUseDefNoPhiUse (n
                                             , use
                                             , def);
  ...
  {
    List_t succs = Graph_successors (theg
                                     , n);
    ...
    while (succs){
      ...
      Ssa_Block_renamePhiArgPre (y
                                 , n
                                 , callBackSubstPhiProp);
      succs = succs->next;
    }
  }
...
static Ssa_Block_t rewriteBlock (Ssa_Block_t b)
{
  ...
  newb = Ssa_Block_renamePhiArgs (newb
                                  , phiargs);
  return newb;
}
...
static Ssa_Fun_t renameVarTraced (Ssa_Fun_t f
                                  , Graph_t g
                                  , Tree_t tree)
{
  ...
  stackProp 
    = Property_newInitFun 
    ((Poly_tyPlist)Id_plist
     , (Poly_tyPropInit)stackPropInitFun);
  substProp = Property_new ((Poly_tyPlist)Ssa_Block_plist);
  substPhiProp 
    = Property_newInitFun 
    ((Poly_tyPlist)Ssa_Block_plist
     , (Poly_tyPropInit)substPhiPropInitFun);

  freshNameProp 
    = Property_newInitFun 
    ((Poly_tyPlist)Id_plist
     , (Poly_tyPropInit)freshNamePropInitFun);
...
static Ssa_Fun_t transFunEach (Ssa_Fun_t f)
{
  ...
  Log_strs ("now ready to translate function: "
            ,Id_toString (f->name)
            , "\n"
            , 0);
  f = insertPhiAndRename (f);  
  Log_strs ("translating function finished: "
            , Id_toString (f->name)
            , "\n"
            , 0); 
  return f;
...
static void printResult (Ssa_Prog_t p)
{
  ...
  File_saveToFile ("consSsa.result"
                   , (Poly_tyPrint)Ssa_Prog_print
                   , p);
  return;
}
...
Ssa_Prog_t Ssa_constructSsa (Ssa_Prog_t p)
{
  ...
  Trace_TRACE("Ssa_constructSsa"
              , Ssa_constructSsaTraced
              , (p)
              , printArg
              , r
              , printResult);
