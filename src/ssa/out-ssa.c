...
static void analyzeOne (Ssa_Block_t b)
{
  ...
  while (stms){
    ...
    
    {
      ...
      Property_set (nameProp
                    , dest
                    , Tuple_new (newdest, newa));
      ...
      while (args){
        ...
        List_insertLast (oldtail
                         , Ssa_Stm_new_move (newa
                                             , opr));
        List_insertLast (oldtail
                         , Ssa_Stm_new_move
                         (newdest
                          , Ssa_Operand_new_id (newa)));
        args = args->next;
      }
      oldhead = Property_get (headProp, b);
      List_insertLast (oldhead
                         , Ssa_Stm_new_move
                         (dest
                          , Ssa_Operand_new_id (newdest)));
    }
...
static Ssa_Prog_t Ssa_outSsaTraced (Ssa_Prog_t p)
{
  ...
  Assert_ASSERT(p);
  headProp 
    = Property_newInitFun ((Poly_tyPlist)Ssa_Block_plist
                           , (Poly_tyId)headPropInitFun);
  tailProp 
    = Property_newInitFun ((Poly_tyPlist)Ssa_Block_plist
                           , (Poly_tyId)tailPropInitFun);
  nameProp
...
static void printArg (Ssa_Prog_t p)
{
  ...
  {
    File_t file = File_open ("outSsa.arg"
                             , "w+");
...
static void printResult (Ssa_Prog_t p)
{
  ...
  // and also print it out
  {
    File_t file = File_open ("outSsa.result"
                             , "w+");
    Ssa_Prog_print (file, p);
    File_close (file);
  }
