...
M Ssa_Mem_renameUse (M m, Id_t(*use)(Id_t))
{
  ...
  case SSA_MEM_ARRAY:
    return Ssa_Mem_new_array
      (use (m->u.array.name)
       , Ssa_Operand_renameUse (m->u.array.index, use));
  case SSA_MEM_CLASS:
...
M Ssa_Mem_renameUse2Op (M m, O(*f)(Id_t))
{
  ...
  case SSA_MEM_ARRAY:
    return Ssa_Mem_new_array
      ...
      (m->u.array.name
       , Ssa_Operand_renameUse2Op (m->u.array.index
                                   , f));
  case SSA_MEM_CLASS:
    return Ssa_Mem_new_class
...
int Ssa_Stm_PhiArg_equals (Ssa_Stm_PhiArg_t a1
                           , Ssa_Stm_PhiArg_t a2)
{
...
void Ssa_Stm_PhiArg_foreachUse (Ssa_Stm_PhiArg_t a
                                , void (*f)(Id_t))
{
...
int Ssa_Stm_PhiArg_isSameConst (List_t l)
{
  ...
  while (l){
    ...
    if (!Ssa_Operand_isConst (current->arg))
      return 0;
    if (!Ssa_Operand_equals (current->arg
                             , firstArg->arg))
      return 0;
    l = l->next;
  }
...
File_t Ssa_Stm_PhiArg_print (File_t file
                           , Ssa_Stm_PhiArg_t arg)
{
  Assert_ASSERT(arg);
...
String_t Ssa_Stm_PhiArg_toString (Ssa_Stm_PhiArg_t arg)
{
  ...
  return String_concat (Ssa_Operand_toString (arg->arg)
                        , " ("
                        , Label_toString (arg->pred->label)
                        , "), "
                        , 0);
}
...
S Ssa_Stm_new_phi_predsBlock (Id_t dest, List_t preds)
{
  ...
  while (preds){
    B b = (B)preds->data;
    Ssa_Stm_PhiArg_t arg 
      = Ssa_Stm_PhiArg_new (Ssa_Operand_new_id (dest)
                            , b);
...
static Set_t Ssa_Stm_getDefIds (S s)
{
  ...
  switch (s->kind){
  case SSA_STM_MOVE:
    return Set_singleton ((Poly_tyEquals)Id_equals
                          , s->u.move.dest);
  case SSA_STM_BOP:
    return Set_singleton ((Poly_tyEquals)Id_equals
                          , s->u.bop.dest);
...
void Ssa_Stm_foreachUse (S s, void (*f)(Id_t))
{
  ...
  switch (s->kind){
  case SSA_STM_MOVE:
    Ssa_Operand_foreachUse (s->u.move.src
                            , f);
    return;
  case SSA_STM_BOP:
    Ssa_Operand_foreachUse (s->u.bop.left
                            , f);
    Ssa_Operand_foreachUse (s->u.bop.right
                            , f);
    return;
  case SSA_STM_UOP:
    Ssa_Operand_foreachUse (s->u.uop.src
                            , f);
    return;
  ...
  case SSA_STM_NEW_CLASS:
    return;
  case SSA_STM_NEW_ARRAY:
    Ssa_Operand_foreachUse (s->u.newArray.size
                            , f);
    return;
    // only rename def
  case SSA_STM_PHI:{
    ...
    while (l){
      Ssa_Stm_PhiArg_foreachUse (l->data
                                 , f);
      l = l->next;
    }
    return;
...
S Ssa_Stm_renameUseDefNoPhiUse (S s)
{
  ...
  case SSA_STM_MOVE:{
    ...
    new = 
      Ssa_Stm_new_move 
      (gud.def (s->u.move.dest)
       , Ssa_Operand_renameUse (s->u.move.src
                                , gud.use));
  ...
  case SSA_STM_BOP:
    return Ssa_Stm_new_bop
      (gud.def (s->u.bop.dest)
       , Ssa_Operand_renameUse (s->u.bop.left
                                , gud.use)
       , s->u.bop.op
       , Ssa_Operand_renameUse (s->u.bop.right
                                , gud.use));
  case SSA_STM_UOP:
    return Ssa_Stm_new_uop
      (gud.def (s->u.uop.dest)
       , s->u.uop.op
       , Ssa_Operand_renameUse (s->u.uop.src
                                , gud.use));
  case SSA_STM_STORE:
    return Ssa_Stm_new_store 
      (Ssa_Mem_renameUse (s->u.store.m, gud.use)
       , Ssa_Operand_renameUse (s->u.store.src, gud.use));
  case SSA_STM_LOAD:
    return Ssa_Stm_new_load
      (gud.def (s->u.load.dest)
       , Ssa_Mem_renameUse (s->u.load.m, gud.use));
  case SSA_STM_NEW_CLASS:
    return Ssa_Stm_new_newClass
      (gud.def (s->u.newClass.dest)
       , s->u.newClass.cname);
  case SSA_STM_NEW_ARRAY:
    return Ssa_Stm_new_newArray 
      (gud.def (s->u.newArray.dest)
       , s->u.newArray.ty
       , Ssa_Operand_renameUse (s->u.newArray.size
                                , gud.use));
    // only rename def
  case SSA_STM_PHI:
    return Ssa_Stm_new_phi
      (gud.def (s->u.phi.dest)
       , s->u.phi.args);
  case SSA_STM_TRY:
  case SSA_STM_TRY_END:
     return s;
...
S Ssa_Stm_renameUse2Op (S s, O (*f)(Id_t))
{
  ...
  case SSA_STM_MOVE:{
    ...
    return 
      Ssa_Stm_new_move 
      (s->u.move.dest
       , Ssa_Operand_renameUse2Op (s->u.move.src
                                   , f));
  }
  case SSA_STM_BOP:{
    if (f (s->u.bop.dest))
    ...
    return Ssa_Stm_new_bop
      (s->u.bop.dest
       , Ssa_Operand_renameUse2Op (s->u.bop.left
                                   , f)
       , s->u.bop.op
       , Ssa_Operand_renameUse2Op (s->u.bop.right
                                   , f));
  }
  case SSA_STM_UOP:{
    if (f (s->u.uop.dest))
    ...
    return Ssa_Stm_new_uop
      (s->u.uop.dest
       , s->u.uop.op
       , Ssa_Operand_renameUse2Op (s->u.uop.src
                                   , f));
  }
  
  case SSA_STM_STORE:
    return Ssa_Stm_new_store 
      (Ssa_Mem_renameUse2Op (s->u.store.m
                             , f)
       , Ssa_Operand_renameUse2Op (s->u.store.src
                                   , f));
  case SSA_STM_LOAD:
    return Ssa_Stm_new_load
      (s->u.load.dest
       , Ssa_Mem_renameUse2Op (s->u.load.m
                               , f));
  case SSA_STM_NEW_CLASS:
    return Ssa_Stm_new_newClass
      (s->u.newClass.dest
  ...
  case SSA_STM_NEW_ARRAY:
    return Ssa_Stm_new_newArray 
      (s->u.newArray.dest
       , s->u.newArray.ty
       , Ssa_Operand_renameUse2Op (s->u.newArray.size
                                   , f));
  ...
  case SSA_STM_PHI:{
    ...
    if ((src = f (s->u.phi.dest)))
      return Ssa_Stm_new_move (s->u.phi.dest
                               , src);

    return Ssa_Stm_new_phi
      (s->u.phi.dest
       , List_map (s->u.phi.args
                   , (Poly_tyId)Ssa_Stm_PhiArg_renameUse2Op));
  }
  case SSA_STM_TRY:
  case SSA_STM_TRY_END:
    return s;
...
S Ssa_Stm_renamePhiArgs (S s)
{
  ...
  while (oldArgs){
    ...
    List_insertLast (newArgs
                     , Ssa_Stm_PhiArg_new
                     (Ssa_Operand_new_id (newid)
                      , a->pred));
    
    oldArgs = oldArgs->next;
  }

  return Ssa_Stm_new_phi (s->u.phi.dest
                          , newArgs);
}
...
File_t Ssa_Stm_print (File_t file, S s)
{
  ...
  case SSA_STM_PHI:{
    ...
    fprintf (file, "%s", " = PHI");
    fprintf (file, "(");
    List_foldl (s->u.phi.args
                , file
                , (Poly_tyFold)Ssa_Stm_PhiArg_print);
    fprintf (file, "%s", ")");
    break;
  }
  default:
...
T Ssa_Transfer_new_call (Id_t dest
                         , Id_t f
                         , List_t args
                         , Label_t leave
                         , Label_t normal)
...
T Ssa_Transfer_renameUse (T t, Id_t (*use)(Id_t))
{
  ...
  case SSA_TRANS_IF:
    return Ssa_Transfer_new_if 
      (Ssa_Operand_renameUse (t->u.iff.cond, use)
       , t->u.iff.truee
       , t->u.iff.falsee);    
  case SSA_TRANS_JUMP:
    return t;
...
static T Ssa_Transfer_renameUseDefNoPhiUse (T t)
{
  ...
  case SSA_TRANS_IF:
    return Ssa_Transfer_new_if 
      (Ssa_Operand_renameUse (t->u.iff.cond
                              , gud.use)
       , t->u.iff.truee
       , t->u.iff.falsee);    
  case SSA_TRANS_JUMP:
...
T Ssa_Transfer_renameUse2Op (T t, O (*f)(Id_t))
{
  ...
  case SSA_TRANS_IF:
    return Ssa_Transfer_new_if 
      (Ssa_Operand_renameUse2Op (t->u.iff.cond, f)
       , t->u.iff.truee
       , t->u.iff.falsee);    
  case SSA_TRANS_JUMP:
    return t;
    ...
    return Ssa_Transfer_new_call
      (t->u.call.dest
       ...
       , List_map (t->u.call.args
                   , (Poly_tyId)Ssa_Operand_renameUse2OpList)
       , t->u.call.leave
       , t->u.call.normal);
  default:
    Error_impossible ();
...
File_t Ssa_Transfer_print (File_t file, T t)
{
  ...
  case SSA_TRANS_CALL:
    ...
    fprintf (file, "(");
    List_print (t->u.call.args
                , ", "
                , file
                , (Poly_tyListPrint)Ssa_Operand_print);
    fprintf (file, "%s", ")");
    fprintf (file, "LEAVE=%s, NORMAL=%s"
             , t->u.call.leave?
...
int Ssa_Block_canJumpTo (B b, Label_t l)
{
  ...
  return Ssa_Transfer_canJumpTo (b->transfer
                                 , l);
}
...
File_t Ssa_Block_print (File_t file, B b)
{
  ...
  List_foldl (b->stms
              , file
              , (Poly_tyFold)Ssa_Stm_print);
  Ssa_Transfer_print (file, b->transfer);
  fprintf (file, "%s", "\n");
  return file;
}

B Ssa_Block_renameUseDefNoPhiUse (B b
                                  , Id_t (*use)(Id_t)
                                  , Id_t (*def)(Id_t))
{
  List_t newStms;
  ...
  newStms = List_map (b->stms
                      , (Poly_tyId)Ssa_Stm_renameUseDefNoPhiUse);
  //newStms = b->stms;
  /* printf ("debug:\n"); */
  /* file = stdout; */
...
void Ssa_Block_renamePhiArgPre (B current
                                , B predessor
                                , void (*f)(B
                                            , B
                                            , Id_t))
...
B Ssa_Block_renamePhiArgs (B b, List_t subst)
{
  ...
  newStms = List_map (b->stms
                      , (Poly_tyId)Ssa_Stm_renamePhiArgs);
  return Ssa_Block_new (b->label
                        , newStms
                        , b->transfer);
}
...
static Set_t Ssa_Block_getDefIdsTraced (B b)
{
  ...
  HashSet_t set = HashSet_new ((int (*)(Poly_t))Id_hashCode
                               , (Poly_tyEquals)Id_equals);
  List_t stms;
  ...
  HashSet_unionVoidSet 
    (set
     , Ssa_Transfer_getDefIds (b->transfer));
...
Set_t Ssa_Block_getDefIds (B b)
{
  ...
  Trace_TRACE("getDefIds"
              , Ssa_Block_getDefIdsTraced
              , (b)
              , dumb
              , r
              , dumb);
...
void Ssa_Fun_toDot (F f, String_t name)
{
  ...
  Graph_toJpgWithName (g
                       , (Poly_tyPrint)Ssa_Block_printForDot
                       , name);
  return;
}
...
Graph_t Ssa_Fun_toGraph (F f)
{
  ...
  g = Graph_newWithName ((Poly_tyEquals)Ssa_Block_equals
                         , Id_toString (f->name));
...
File_t Ssa_Prog_print (File_t file, P x)
{
  ...
  List_foldl (x->classes
              , file
              , (Poly_tyFold)Class_print);
  fprintf (file, "\n");
  List_foldl (x->funcs
              , file
              , (Poly_tyFold)Ssa_Fun_print);
  return file;
}
